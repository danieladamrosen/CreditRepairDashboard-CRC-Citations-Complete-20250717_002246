...
export function AccountRow({
  ...
  // ✅ STEP 4: Add account rendering verification logging
  const accountId = account['@CreditLiabilityID'] || account['@_AccountIdentifier'] || '';
  const accountName = account._CREDITOR?.['@_Name'] || 'Unknown Account';
  const accountType = account['@_AccountType'] || 'Unknown';
  const accountStatus = account['@_AccountStatusType'] || 'Unknown';
  
  React.useEffect(() => {
    console.log(`Rendering account row: ${accountName} (${accountId}) - Status: ${accountStatus} - Type: ${accountType}`);
    console.log(`AI Violations for ${accountId}:`, aiViolations);
    console.log(`AI Scan Completed:`, aiScanCompleted);
  }, [accountName, accountId, accountStatus, accountType, aiViolations, aiScanCompleted]);

  // Helper function to get violation count text with structured violations
  const getViolationCountText = (isDesktop: boolean = true) => {
    // Use new separated violations if available, otherwise fall back to aiViolations
    const metro2Count = metro2Violations.length > 0 ? metro2Violations.length : aiViolations.filter((v) => v.includes('Metro 2')).length;
    const fcrFdcpaCount = fcrFdcpaViolations.length > 0 ? fcrFdcpaViolations.length : aiViolations.filter((v) => v.includes('FCRA') || v.includes('FDCPA')).length;
    
    if (metro2Count > 0 && fcrFdcpaCount > 0) {
      return isDesktop ? ` (${metro2Count} Metro 2, ${fcrFdcpaCount} FCRA/FDCPA)` : ` (${metro2Count}M2, ${fcrFdcpaCount}F)`;
    } else if (metro2Count > 0) {
      return isDesktop ? ` (${metro2Count} Metro 2)` : ` (${metro2Count}M2)`;
    } else if (fcrFdcpaCount > 0) {
      return isDesktop ? ` (${fcrFdcpaCount} FCRA/FDCPA)` : ` (${fcrFdcpaCount}F)`;
    }
    return '';
  };
  ...
  const [guidedHelpSuggestions, setGuidedHelpSuggestions] = useState<string[]>([]);
  ...
  return (
    ...
    >
      ...
      <CardContent className={`${hasAnyNegative ? (isDisputeSaved ? 'bg-green-50' : 'bg-rose-50') : 'bg-white'} px-4 md:px-6 ${hasAnyNegative ? 'pt-6 pb-6' : 'pt-1 pb-2'}`}>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 mb-4 items-stretch ${isFirstCopy ? 'gap-6' : 'gap-4'}`}>
          {/* TransUnion */}
          <div className="relative">
            {account.reportingBureaus?.TransUnion === false ? (
          ...
          {/* Equifax */}
          <div>
            {account.reportingBureaus?.Equifax === false ? (
          ...
          {/* Experian */}
          <div>
            {account.reportingBureaus?.Experian === false ? (
      ...
      {/* Metro 2 Violations and Dispute Suggestions - Only show above dispute module after AI scan */}
        ...
        <div className="px-4 md:px-6 py-4 space-y-4">
          ...
          <div style={{ marginTop: '-6px' }}>
            ...
            >
              ...
              <span>
                View {(metro2Violations.length + fcrFdcpaViolations.length) || aiViolations.length} Compliance Violations
                              ...
                              <span
                                className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-xs font-medium ${
                                  violation.includes('Metro 2')
                                    ...
                                    : violation.includes('FCRA') 
                                      ? 'bg-red-200 text-red-800 border border-red-300'
                                      : 'bg-orange-200 text-orange-800 border border-orange-300'
                                }`}
                                style={{ fontSize: '10px' }}
                              >
                                {violation.includes('Metro 2') ? 'Metro 2' : 
                                 violation.includes('FCRA') ? 'FCRA' : 'FDCPA'}
                              </span>
                              <span className="text-sm font-medium">{violation}</span>
                            </div>
                            {showViolationSources && (
                              <div className="ml-4 text-xs text-gray-500 italic">
                                Source: {violation.includes('Metro 2') ? 'CRC Advanced Disputing Workbook, Page 3-46' : 
                                        violation.includes('FCRA') ? 'CRC Basic Disputing Course, Page 1-15' : 
                                        'CRC Top 10 Advanced Dispute Letters, Page 1-292'}
                              </div>
                            )}
                          </div>
                          <Button
                        ...
                        <div className="md:hidden">
                          <div className="flex items-center gap-2 mb-2">
                            <span
                              ...
                              style={{ fontSize: '10px' }}
                            >
                              {violation.includes('Metro 2') ? 'M-2' : 
                               violation.includes('FCRA') ? 'FCRA' : 'FDCPA'}
                            </span>
                            <span className="text-sm font-medium">{violation}</span>
                          </div>
                          {showViolationSources && (
                            <div className="ml-4 text-xs text-gray-500 italic mb-2">
                              Source: {violation.includes('Metro 2') ? 'CRC Advanced Disputing Workbook, Page 3-46' : 
                                      violation.includes('FCRA') ? 'CRC Basic Disputing Course, Page 1-15' : 
                                      'CRC Top 10 Advanced Dispute Letters, Page 1-292'}
                            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 justify-between items-center pt-2">
                ...
                <div className="flex items-center gap-2 relative overflow-visible">
                  ...
                  <Button
                    disabled={!isDisputeSaved && !isFormComplete()}
                    onClick={() => {
                      ...
                      function proceedWithAccountSave() {
                        ...
                        if (onDisputeSaved) {
                          ...
                          onDisputeSaved(accountId, {
                            ...
                            // Store metadata about input source for proper UI display
                            reasonSource: (selectedViolations.length > 0 || showCustomReasonField) ? 'custom' : 'dropdown',
                            instructionSource: (selectedViolations.length > 0 || showCustomInstructionField) ? 'custom' : 'dropdown',
                          });
                        }
