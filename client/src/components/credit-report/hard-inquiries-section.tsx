...
export function HardInquiriesSection({
  ...
  // Calculate saved disputes count (only for saved inquiries)
  const calculateSavedDisputesCount = () => {
    let recentSavedCount = 0;
    let olderSavedCount = 0;
    
    // Count recent inquiry saved disputes
    if (recentInquiriesSaved && recentInquirySelections.length > 0) {
      recentSavedCount = recentInquirySelections.length;
    }
    
    // Count older inquiry saved disputes
    if (olderInquiriesSaved && olderInquirySelections.length > 0) {
      olderSavedCount = olderInquirySelections.length;
    }
    
    const totalSavedCount = recentSavedCount + olderSavedCount;
    console.log(`Hard Inquiries saved count: ${totalSavedCount} (Recent: ${recentSavedCount}, Older: ${olderSavedCount})`);
    
    return totalSavedCount;
  };

  // Get bureaus with saved disputes
  const getSavedBureauNames = () => {
    const savedBureaus = new Set<string>();
    
    // Add bureaus from recent inquiry selections if saved
    if (recentInquiriesSaved && recentInquirySelections.length > 0) {
      recentInquirySelections.forEach(selection => {
        if (selection.bureau === 'TU') savedBureaus.add('TransUnion');
        if (selection.bureau === 'EQ') savedBureaus.add('Equifax');
        if (selection.bureau === 'EX') savedBureaus.add('Experian');
      });
    }
    
    // Add bureaus from older inquiry selections if saved
    if (olderInquiriesSaved && olderInquirySelections.length > 0) {
      olderInquirySelections.forEach(selection => {
        if (selection.bureau === 'TU') savedBureaus.add('TransUnion');
        if (selection.bureau === 'EQ') savedBureaus.add('Equifax');
        if (selection.bureau === 'EX') savedBureaus.add('Experian');
      });
    ...
    console.log(`Saved bureaus for Hard Inquiries: [${bureauArray.join(', ')}]`);
  ...
  const savedDisputesCount = calculateSavedDisputesCount();
  const savedBureauNames = getSavedBureauNames();
  const bureauText = formatBureauNames(savedBureauNames);
  
  // Console logging for Hard Inquiries verification
  console.log("Hard Inquiries - Saved disputes count:", savedDisputesCount, "| Bureaus:", savedBureauNames);
  ...
  // Callback function to reset choreography flags for re-trigger
  const handleRecentChoreographyReset = () => {
    console.log("🔄 Recent Inquiries Re-trigger: Resetting choreography flags");
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${
                  ...
                  {allInquiriesSaved || hardInquiriesGreenShown ? 
                    `You've saved disputes for ${savedDisputesCount} ${savedDisputesCount === 1 ? 'inquiry' : 'inquiries'} across ${bureauText || 'TransUnion, Equifax, and Experian'}.` :
          ...
          {showHardInquiries && (
            <CardContent className={allInquiriesSaved || hardInquiriesGreenShown ? 'bg-green-50' : 'bg-white'}>
              <Inquiries
                ...
                onRecentChoreographyReset={handleRecentChoreographyReset}
              />
            </CardContent>
          )}
        </Card>
