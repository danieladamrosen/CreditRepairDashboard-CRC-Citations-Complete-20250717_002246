...
interface InquiriesProps {
  ...
  // Re-trigger choreography callback
  onRecentChoreographyReset?: () => void;
}
...
export function Inquiries({
  ...
  // AI toggle states for Recent Inquiries
  const [showRecentViolations, setShowRecentViolations] = useState(false);
  ...
  // States for Show Sources toggle functionality
  ...
  // Helper function to get saved bureau names for recent inquiries
  const getRecentSavedBureauNames = () => {
    const savedInquiryKeys = Object.keys(selectedRecentInquiries).filter(key => selectedRecentInquiries[key]);
    const bureauCodes = new Set<string>();
    
    savedInquiryKeys.forEach(key => {
      const [, bureau] = key.split('-');
      if (bureau) {
        bureauCodes.add(bureau);
      }
    });
    
    const bureauNames = Array.from(bureauCodes).map(code => {
      switch (code) {
        case 'TU': return 'TransUnion';
        case 'EQ': return 'Equifax';
        case 'EX': return 'Experian';
        default: return code;
      }
    });
    
    console.log("Recent Inquiries Saved Disputes Count:", savedInquiryKeys.length, "| Bureaus:", bureauNames);
    return bureauNames;
  };

  // Helper function to format bureau names for display
                    ...
                    className={`
                      ...
                      ${
                        isDisputeSaved
                          ...
                          : isSelected
                            ? 'border-[3px] border-red-500 bg-white'
  ...
  // Helper function to execute Recent Inquiries save logic
  const executeRecentSaveLogic = () => {
    const disputeData = {
  ...
  // Helper function to execute Older Inquiries save logic
  const executeOlderSaveLogic = () => {
              ...
              <Button
                onClick={() => {
                  ...
                  if (isDisputeSavedState) {
                    ...
                    // Reset state first, then re-execute the full save logic
                    if (isRecent) {
                      setIsRecentDisputeSaved(false);
                      setRecentSaved(false);
                      setTimeout(() => {
                        // Re-execute the full Recent Inquiries save logic
                        executeRecentSaveLogic();
                      }, 50);
                    } else {
                      setIsOlderDisputeSaved(false);
                      setOlderSaved(false);
                      setTimeout(() => {
                        // Re-execute the full Older Inquiries save logic
                        executeOlderSaveLogic();
                      }, 50);
                    }
                    return;
                  }

                  // Execute the first-time save logic
                  if (isRecent) {
                    executeRecentSaveLogic();
                  } else {
                    executeOlderSaveLogic();
                ...
                disabled={!isDisputeSavedState && (!Object.values(selectedItems).some(Boolean) || !reason || !instruction)}
                className={`${
                  isDisputeSavedState
          ...
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className={isRecentDisputeSaved ? "mt-0.5" : ""}>
                    ...
                    <p className={`text-sm ${isRecentDisputeSaved ? 'text-green-700' : 'text-orange-600 font-medium tracking-tight'}`}>
                      {isRecentDisputeSaved
                        ? `You've saved disputes for ${Object.values(selectedRecentInquiries).filter(Boolean).length} inquiries across ${formatBureauNames(getRecentSavedBureauNames()) || 'TransUnion, Equifax, and Experian'}.`
            ...
            {effectiveShowRecentInquiries && (
              <div className={`w-[1150px] max-w-[1150px] mx-auto rounded-lg p-6 ${isRecentDisputeSaved ? 'bg-green-50 border-[2px] border-green-500' : Object.values(selectedRecentInquiries).some(Boolean) ? 'bg-rose-50 border-[2px] border-red-500' : 'bg-gray-50 border-[2px] border-gray-300'}`}>
                ...
                {aiScanCompleted && Object.values(selectedRecentInquiries).some(Boolean) && (
                  <div className="mt-4 space-y-4">
                    {/* View Compliance Violations Link */}
                    {aiViolations && Object.keys(aiViolations).length > 0 && (
                      <div style={{ marginTop: '-6px' }}>
                        <button
                          onClick={() => {
                            setShowRecentViolations(!showRecentViolations);
                            if (!showRecentViolations) {
                              setShowRecentSuggestions(false); // Close suggestions when opening violations
                            }
                          }}
                        ...
                        >
                          ...
                          <span>
                            View {Object.values(aiViolations).flat().length} Compliance Violations
                          </span>
                          {showRecentViolations ? (
                        ...
                        {showRecentViolations && (
                          ...
                          >
                            <div className="mb-3 flex items-center justify-between">
                              <button
                                onClick={() => setShowRecentViolations(!showRecentViolations)}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  onClick={async (e) => {
                                    ...
                                    const allViolationsText = Object.values(aiViolations).flat().join(' ');
                                ...
                                >
                                  <Zap className="w-3 h-3 mr-1 flex-shrink-0" />
                                  Use All {Object.values(aiViolations).flat().length}
                                </Button>
                              </div>
                            </div>
                            <div className="mt-4 space-y-2">
                              {Object.values(aiViolations).flat().map((violation, index) => (
                                <div key={index} className="p-3 bg-white rounded border border-gray-200">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      {showViolationSources && (
                                        <div className="ml-4 text-xs text-gray-500 italic">
                                          Source: {violation.includes('Metro 2') ? 'CRC Advanced Disputing Workbook, Page 3-46' : 'CRC Basic Disputing Course, Page 1-15'}
                                        </div>
                                      )}
                                      </div>
                                      <Button
                                        ...
                                        onClick={async (e) => {
                                          e.stopPropagation();
                                          // Auto-populate both fields with this specific violation
                                          const reason = violation.trim();
                                          const instruction = violation.trim();
                      ...
                      {aiSuggestions && Object.keys(aiSuggestions).length > 0 && (
                        <div style={{ marginTop: '-2px' }}>
                          <button
                            onClick={() => {
                              setShowRecentSuggestions(!showRecentSuggestions);
                              if (!showRecentSuggestions) {
                                setShowRecentViolations(false); // Close violations when opening suggestions
                              }
                            }}
                            className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 p-2 rounded-md transition-colors font-medium"
                          >
                            ...
                            >
                              <div className="mb-3 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    onClick={() => setShowRecentSuggestions(false)}
                              ...
                              <div className="mt-4 space-y-2">
                                {Object.values(aiSuggestions).flat().map((suggestion, index) => (
                                  <div key={index} className="p-3 bg-white rounded border border-gray-200">
                                    <div className="flex justify-between items-center">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                      ...
                                      <Button
                                        ...
                                        onClick={async (e) => {
                                          e.stopPropagation();
                                          // Auto-populate both fields with this specific suggestion
                                          const reason = suggestion.trim();
                                          const instruction = suggestion.trim();
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-base font-bold ${isRecentDisputeSaved ? 'text-green-600' : 'text-gray-700'}`}>
                        {isRecentDisputeSaved ? 'Dispute Created' : 'Create your dispute'}
                      </span>
                    </div>
                  </div>
